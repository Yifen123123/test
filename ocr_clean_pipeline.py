...
# -*- coding: utf-8 -*-
"""
OCR .txt 清洗流程（無 LanguageTool 版本，免 Java）：
1) clean-text 去除控制字元/亂碼與基礎正規化
2) OpenCC 簡繁統一（預設：t2tw，簡→正體台灣慣用字）
3) 第二層整理：Regex 合併錯誤換行、標點規整、中文斷句
4) 基本補標點（規則式；無語法檢查）

輸入：.txt
輸出：同名 *.clean.txt（或 -o 指定）
...
def basic_punct_fix(sents):
    ...
    fixed = []
    for s in sents:
        # 段首保留：若為段落標頭，直接入列（不補句點）
        if SECTION_HEAD_PAT.match(s):
            # 統一「：」為全形已在 normalize_punct 做過，這裡只確保行尾不亂補。
...
def main():
    ap = argparse.ArgumentParser(description="OCR .txt 清洗（無 LanguageTool 版：clean-text → OpenCC → Regex斷句 → 規則補標點）")
    ...
    print("[INFO] 第一層：clean-text + OpenCC…")
