...
<script>
const RAW_ROOT = {json.dumps(args.raw_root)!r};
const CLASS_COLORS = {json.dumps(CLASS_COLORS, ensure_ascii=False)};
...
function render() {{
  ...
  tb.innerHTML = rows.map(d => `
    <tr>
      ...
      <td>${{dueChip(d)}}</td>
      <td>${{htmlEscape(d.agency||"-")}}</td>
      <td>${{htmlEscape(d.case_id||"-")}}</td>
      <td>
        <div class="chips">${{(d._who_str||"").split("；").filter(Boolean).map(x=>`<span>${{htmlEscape(x)}}</span>`).join(" ")}}</div>
        <div class="muted">${{htmlEscape(d._ids_str||"")}}</div>
      </td>
      <td>
        <div><strong>${{htmlEscape(d.subject||"")}}</strong></div>
        <div class="muted" style="margin-top:4px">${{htmlEscape(d.summary_text||"")}}</div>
      </td>
    </tr>
  `).join("");
